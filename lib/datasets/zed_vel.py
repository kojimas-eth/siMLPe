...
class ZEDDataset(data.Dataset):
    def __init__(self, config, split_name, data_aug=False):
        ...
        # Load data
        self.zed_seqs, self.zed_vels, self.zed_yaw_rates = self._load_zed_data()
    ...
    def _load_zed_data(self):
        ...
        for folder in source_dir:
            ...
            if hasattr(self.config, 'action_filter') and len(self.config.action_filter) > 0:
                filtered_list = []
                for f in file_list:
                    filename = os.path.basename(f).lower()
                
                    # Keep the file if ANY of the keywords are in the filename
                    if any(keyword.lower() in filename for keyword in self.config.action_filter):
        ...
        for file_path in files_to_load:
            with open(file_path, 'r') as f:
                ...
                for ts in sorted_timestamps[:-30]:
                    ...
                    if not frame_data.get('body_list'):
                        if zed_keypoints:
                            ...
                            zed_quats.append(zed_quats[-1])             # Add this!
                            zed_velocities.append(zed_velocities[-1])
                        continue
                    ...
                    vel = target_body.get('velocity', [0.0,0.0,0.0])
                    zed_quats.append(quat)
                    zed_velocities.append(vel)

                zed_keypoints_np = np.array(zed_keypoints)  # (Total frames, 34, 3)
                zed_quats_np = np.array(zed_quats)
                zed_velocities_np = np.array(zed_velocities) # (Total frames, 3)
                zed_velocities_np[:, 0] = -zed_velocities_np[:, 0] # Flip X
                zed_velocities_np[:, 2] = -zed_velocities_np[:, 2] # Flip Z
                vel_xz = zed_velocities_np[:, [0, 2]]
                ...
                euler_angles = rotations.as_euler('yxz', degrees=False) # Get angles in radians
                yaw_angles = euler_angles[:, 0] #spin
                
                yaw_velocity = np.zeros_like(yaw_angles)
                yaw_velocity[1:] = yaw_angles[1:] - yaw_angles[:-1]
            ...
            all_velocities.append(vel_xz)
            all_yaw_rates.append(yaw_velocity)

        final_dataset= np.concatenate(all_sequences, axis=0)
        final_velocities = np.concatenate(all_velocities, axis=0)
        final_yaw_rates = np.concatenate(all_yaw_rates, axis=0)
        print(f"Combined zed final dataset shape is {final_dataset.shape}")
        return [final_dataset], [final_velocities], [final_yaw_rates]
    ...
    def __getitem__(self, index):
        ...
        velocity = self.zed_vels[seq_idx][start_frame:end_frame]
        yaw_rates= self.zed_yaw_rates[seq_idx][start_frame:end_frame]

        # Data Augmentation (Time Reversal)
        if self.data_aug and self.split_name == 'train':
            if torch.rand(1)[0] > 0.5:
                motion = np.flip(motion, axis=0).copy()
                velocity = np.flip(velocity, axis=0).copy()
                velocity = -velocity
                yaw_rates = np.flip(yaw_rates, axis=0).copy()
                yaw_rates = -yaw_rates

        # Convert to Tensor
        motion = torch.from_numpy(motion).float()
        velocity = torch.from_numpy(velocity).float()
        yaw_rates = torch.from_numpy(yaw_rates).float()

        h36m_motion_input = motion[:self.input_len]
        h36m_motion_target = motion[self.input_len:] 
        velocity_target = velocity[self.input_len:]
        yaw_rates_target = yaw_rates[self.input_len:]
        ...
        if len(yaw_rates_target.shape) == 1:
            yaw_rates_target = yaw_rates_target.unsqueeze(-1)

        return h36m_motion_input, h36m_motion_target, velocity_target, yaw_rates_target