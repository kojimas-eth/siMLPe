...
#######################
#Load h36M data directly
########################
data = np.loadtxt("/home/sosuke/thesis/siMLPe/data/h36m/S5/discussion_1.txt", delimiter=",")
print(f"Loaded shape: {data.shape}")
dataset = H36MEval(config, 'test')
input,output = dataset[0]
print("Sample from H36M Eval Dataset")
print("Input shape:", input.shape)   # Expected: (50, 32, 3)
print("Output shape:", output.shape)  # Expected: (25, 32,3)
time.sleep(3)
...
for i in range(input.shape[0]):
    current_frame = input[i,:,:]  #(32,3)

    history_buffer.append(current_frame)
    if len(history_buffer)> 50:
        history_buffer.pop(0)

    if len(history_buffer) == 50:
        past_frames = np.array(history_buffer) #(50,32,3)
        # h36m_order_converted = zed34_to_h36m32(past_frames)  #(50,32,3)
        h36m_order_converted = past_frames
...
print(f"Done! Saved {len(all_preds_saved)} samples to sanity_ceck.npz")
